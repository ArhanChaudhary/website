{/* cspell:ignore typestate generativity newtype newtypes newtyped newtyping Rustaceans Rovynak rubiks stickered subtyping rustc binarycat typetoken eyepatch Launchbury Peyton Yanovski nilly unergonomic interprocedural amping brandedvec phantomdata rustonomicon Polonius nonunifiable */}
...
[^unique]: This lifetime is not technically unique. You could unify it with other lifetimes in a function call: `fn unify_lifetimes<'id>(impostor: &'id (), guard: &Guard<'id>) { ... }`. The lifetime is only unique among the provided `Id<'_>` and `Guard<'_>` types, so as long as your lifetime is carried by those types your code will be sound.
...
- Thus, the unique integer identifier is used to test permutation group membership during permutation composition, erroring if not the case. We will momentarily see how the generativity pattern correctly and equivalently solves this problem.

With their equivalence established, the body of `PermGroup::new` hardly changes at all.
...
The purpose of `Guard<'_>` when `Id<'_>` already exists becomes clear when considering that `third` is rejected by the compiler. If `PermGroup::new` accepted an `Id<'_>`, two different permutation groups could be assigned the same `Id<'_>` because it implements `Copy`.
...
- The compiler errors from misusing `generativity` don't provide the location of the error. They are generally confusing, even when you know how it internally works.
...
If you've only used `PhantomData` when the compiler has told you to, this certainly looks nonsensical. The purpose of `Id` as we saw earlier is to carry a *unique* lifetime brand among different permutation groups, but aren't lifetimes already unique? What's the deal?

In Rust, *variance* determines whether you can substitute one lifetime for another. If you have a longer lifetime, Rust lets you use it where a shorter one is expected. This is also known as *subtyping*. Normally this is good—subtyping introduces static analysis that allows for more programs to compile—but in our case this automatic substitution works against our favor. If an `Id<'_>` can be tied to a lifetime other than its lifetime brand, we lose. So, the unique lifetime `'id` must have no subtyping relation with other lifetimes; `'id` is what's called an *invariant* lifetime.

The contemporary usage of the word "invariant" by Rustaceans has two meanings: one as a type-level guarantee (a noun), and one as a no-subtyping relation (an adjective). Invariant generally means something that cannot change or must be fixed to a specific value. If you stretch this definition, both meanings refer to this same general concept. We've been working with the first meaning so far, but for the rest of this article we'll switch to the second one.
...
They work because they follow the core principal that `&mut T` is invariant over `T` because mutable references grant exclusive write access to the data they point to (more in the [Rustonomicon](https://doc.rust-lang.org/nomicon/subtyping.html)). With `T = &'id ()`, `'id` must become invariant. But unlike the others, `PhantomData<fn(&'id ()) -> &'id ()>` implements all auto traits, and is preferred if the only purpose is to indicate invariance. Using this carries further implication of "by example" use of `PhantomData`.

</Aside>

When I first learned how to use `PhantomData` to indicate variance I couldn't help but think of it as an obnoxiously leaky abstraction. There is a [movement](https://github.com/rust-lang/rust/issues/135806) to make it a bit more ergonomic by introducing custom variance newtypes into the standard library. Sure enough, the status quo of `PhantomData` has been considered "[something of a failure](https://github.com/rust-lang/rfcs/pull/3417#pullrequestreview-1396551771)."
...
The entire implementation of `Guard<'_>` is a newtype around `Id<'_>`. The established difference being that `Guard<'_>` doesn't implement `Copy` or `Clone`. We provide a `From` implementation to consume a `Guard<'_>` into an `Id<'_>` to create distributable copies of the lifetime brand, as we saw earlier.
...
If you still find it confusing, I encourage you to work out rustc's error message [in this example](https://play.rust-lang.org/?version=stable&mode=debug&edition=2024&gist=649d51907c2612c310eb627a0c863399). I also encourage you to play around with the code snippet to see what compiles and what doesn't. Even with my own explanation in mind, I find this exercise illuminating. Another tip to help you understand is to remember that lifetimes are descriptive, not prescriptive.

## Soundness

It is easy to verify

- the compiler conservatively assumes that all generic parameters of the Drop impl are used
- The precise rules that govern drop checking may be less restrictive in the future.
- The current analysis is deliberately conservative and trivial; it forces all borrowed data in a value to outlive that value, which is certainly sound. Future versions of the language may make the analysis more precise, to reduce the number of cases where sound code is rejected as unsafe.
- In the meantime, there is an unstable attribute that one can use to assert (unsafely) that a generic type's destructor is guaranteed to not access any expired data, even if its type gives it the capability to do so. That attribute is called may_dangle and was introduced in RFC 1327. is the reason `Box<&'a T>` can be dropped at end of scope despite the `&'a T` borrow being invalidated beforehand.
...
- what is drop ck https://github.com/rust-lang/rfcs/pull/1327
- stabilize may_dangle https://github.com/rust-lang/rfcs/pull/3390
- improve may_dangle https://github.com/rust-lang/rfcs/pull/3417

This relies on dead code to impact borrow checking. In theory, a sufficiently advanced borrow checker looking at the CFG after some inlining would be able to see that dropping lifetime_brand doesn't require the captured lifetime to be live, and destroy the uniqueness guarantee which we've created.

`scopeguard`, a crate with hundreds of millions of downloads, [also relies](https://docs.rs/scopeguard/latest/src/scopeguard/lib.rs.html#287) on the `Drop` glue code of an unused variable

with Polonius we will hopefully have a formal model of the borrow checker and then proving this would be a fun project
...
I'm not going to pretend this is well-thought out
