...
- Although there's only a single line marked unsafe in our `Permutation<'id>` example, its soundness is now much harder to justify. It is on the developer to prove that `'id` uniquely brands permutations within a permutation group; any mishandling could easily make the whole thing unsound (i.e. if `PermGroup::new` took an `Id<'id>` instead of a `Guard<'id>`).
- The `'id` lifetime, like all other lifetimes, is pervasive. Data structures that wish to store `Guard<'_>` or `Id<'_>`, or any other data structure that stores `Guard<'_>` or `Id<'_>`, must have lifetime annotations. But `make_guard` is typically invoked at the outermost scope, and will likely have to be passed through many types, so the number of lifetime annotations is amplified.
- The compiler errors from misusing lifetime branding are confusing, even with knowledge of how `generativity` works internally,
