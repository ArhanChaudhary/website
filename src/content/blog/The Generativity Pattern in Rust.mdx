...
The flaw is quite subtle. The [implementation](https://github.com/ArhanChaudhary/generativity-pattern-rs/blob/main/src/6-unsound-token.rs) creates a token per-*call* instead of per-*owner*. Every expression results in a particular type; if the macro call is ran more than once, it will produce the same type. This can be exploited to give multiple owners the same brand.
...
We have just invoked undefined behavior from safe user-facing code. This is unsound with question. There is no point in endorsing this approach.
...
I hope your found our little survey of design patterns interesting. While generativity is indeed a flashy approach to the problem it is trying to solve, . Do not feel let down—I am almost certain you learned something new about writing Rust.
...
<sup id="1">1</sup> The center stickers don't actually move, and thus can be ignored, so the illustrated turn is usually simplified to a permutation forty-eight elements long. <a href="#1-back" class="back-arrow">↩</a>
