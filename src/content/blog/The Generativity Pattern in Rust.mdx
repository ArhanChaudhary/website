...
`generativity` publicizes three things: `Guard`, `Id`, and `make_guard`. Invoking the `make_guard` macro creates a `Guard<'_>`, a guaranteed unique identifier that does not implement `Copy` or `Clone`. `Id<'_>`, however, does; to create distributable copies of this identifier, you must consume a `Guard<'_>` into an `Id<'_>` using its `From` implementation. In terms of what `generativity` has to offer, this is it.
...
Why is `guard` passed as an argument, and why isn't `make_guard` creating it within the function body? This reveals `generativity`'s caveat: a `Guard<'_>` can never escape the scope it was defined in. Think of creating a `Guard<'_>` as creating a reference to a local variable: no matter what, it is only valid inside of its enclosing scope.
...
We have arrived at the crux of this article. Thousands of words later, bask in our ultimate permutation composition implementation—one that is safe, unchecked, and sound because only `Permutation`s with the same `Id<'id>` can be composed. Behold: the divine elegance of Rust's type system.
