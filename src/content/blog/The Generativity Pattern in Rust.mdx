...
The *generativity pattern* in Rust is a combination of [typestate](https://cliffle.com/blog/rust-typestate/) and [GhostCell](https://plv.mpi-sws.org/rustbelt/ghostcell/), techniques that eliminate runtime overhead through compile-time safety guarantees. This pattern is not commonplace; its usage warrants a specific set of circumstances. Aside from (admittedly well-written) academic literature, I haven't a found an accessible analysis of this pattern online. Contrary to its name, this post won't be about the generativity pattern until the end. In order to build up a full picture of the "what," and more importantly the "why," we will comprehensively walk through a realistic example and gradually build an intuition of the type of problems generativity can solve.
...
<ContentImage src={rubiksCube} width="300" alt="The solved Rubik's cube" />

It follows that if you compose two permutations in a permutation group, the resulting permutation will also be a permutation in that group.
...
There is a more fundamental reason to care. If the permutations to compose are in the same permutation group, the resulting permutation is also in that group. Even if the lengths of permutations from two different groups *did* match, composing them could produce a permutation outside of either permutation group, which is a logic error. Checking if a permutation is a member of a permutation group is a very expensive operation; this is the "practical scenario" I mentioned beforehand.

We will analyze different approaches that ensure our library only permits permutation composition from the same permutation group. Each has their own trade-offs, but are all right answers for their own dedicated situations.
...
The second approach is to use a private integer identifier and validate *upfront* that a set of permutations to avoid the TODO avoid unsafe, If you've read this far and are still interested, I encourage you to internalize
...
impl<'id> PermGroup<'id> {
    ...
    pub fn permutation_from_mapping(
        ...
        let permutation = Permutation::from_mapping(mapping, self.id)?;
        // ... validate that `permutation` is a member of
        // this group
        Ok(permutation)
    }
...
As this StackOverflow user [points out](https://stackoverflow.com/questions/76847647/compile-time-tracking-of-value-owner-with-lifetimes), you can achieve something similar using anonymous "invariant token" unit structs.
...
macro_rules! new_perm_group {
    ($($args:tt)*) => {
        unsafe {
            ...
            $crate::PermGroup::<InvariantToken>::new($($args)*)
        }
    }
}
```

This is unsound with question. No point endorsing this approach.

```rust
// let p1 = new_perm_group!(3, vec![vec![2, 0, 1]]).unwrap();
// let p2 = new_perm_group!(4, vec![vec![1, 2, 0, 3]]).unwrap();
// p1.base_permutations()[0].compose(&p2.base_permutations()[0]);
let mut a = vec![];
for args in [(3, vec![vec![2, 0, 1]]), (4, vec![vec![1, 2, 0, 3]])] {
    a.push(new_perm_group!(args.0, args.1).unwrap());
}
...
| 2-newtype | 4.246
| 4-atomic-id | 4.060
| 5-generativity | 3.546
| 3-unsafe-trait | 3.534

# How does generativity work?

SCRATCH NOTES:

It is [generally agreed upon](https://users.rust-lang.org/t/should-i-use-unsafe-merely-to-encourage-users-to-maintain-invariants/27856) that unsafe should not be used for the sole purpose of maintaining validity invariants.

sticker the cube facelets

underscore that unsafe is to find the location of the error
...
BrandedVec "This implies that we, as the developer of the library, have
the obligation to prove that such accesses are actually safe under all possible interactions with
well-typed clients." This is not so simple
...
https://stackoverflow.com/a/75233099/12230735
