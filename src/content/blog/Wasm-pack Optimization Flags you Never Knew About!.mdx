...
For most people, `-O4` is enough, case closed, but it is easy to overlook an important detail. `wasm-opt` isn't a tool from Rust, but rather a Rust-facing wrapper around `binaryen`'s `wasm-opt`, the C++ compiler and toolchain infrastructure that actually does the meat of the work. You never could have known this if you were new to rustwasm, as it's omitted from the documentation.

You have to dig through source to find the actual list of `binaryen`'s `wasm-opt` optimization flags. Bizarrely, the only mention of the full list is within the test cases.
...
We've uncovered a few more flags that are actually useful in extreme optimization cases. Namely, in my case, the flag `--flexible-inline-max-function-size` increased my WebAssembly file size from 67KB to 83KB while boasting a respectful **40%** speed improvement!
...
These are just the "Optimization options". There exists an even *larger* list of around 150 "Optimization passes" within the same file. Most of them only have a few words to their description, making them difficult to understand.
...
I spent almost an entire day testing out the effectiveness of each flag, to little avail. A lot of these flags weren't even optimizations, and the ones that were were seemingly ineffectual. If you're looking to hyper-optimize your WebAssembly, you're better off sticking to the "Optimization options" flags.

Check out the full list [here](https://github.com/WebAssembly/binaryen/blob/version_117/test/lit/help/wasm-opt.test). At the time of writing, `wasm-pack` is locked to `binaryen` version 117, so this list may soon become outdated. I've also opened a [pull request](https://github.com/rustwasm/wasm-pack/pull/1423) that explicitly references the full list of available `wasm-opt` flags in documentation.
