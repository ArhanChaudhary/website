...
You have to dig through source to find the full list of `binaryen`'s `wasm-opt` optimization flags. Bizarrely, the only mention of the list is within a test case file.
...
Shrouded within the codebase, these few extra flags are actually useful in extreme optimization cases. Namely, in my case, `--flexible-inline-max-function-size` boasted a respectful **40%** speed improvement while increasing my WebAssembly file size from 67KB to 83KB.
...
I spent almost an entire day testing out the effectiveness of each flag, to little avail. A lot of these flags weren't even optimizations, and the ones that were were all seemingly ineffectual. If you're looking to hyper-optimize your WebAssembly, you're better off sticking to the "Optimization options" flags.
