...
- The feature cannot be something you would already expect to compile, at my own discretion.
  ...
  In other words, this feature cannot be an obvious improvement of another feature, possibly originally restricted due to implementation difficulty. This includes [the entire const generics project](https://github.com/rust-lang/project-const-generics), [const traits](https://github.com/rust-lang/rust/issues/143874) and [expression attributes](https://github.com/rust-lang/rust/issues/15701). The intent is to keep the discussion of ideas fresh and interesting.
...
<FeatureSummary title="Associated type defaults">
    <Fragment slot="motivation">
        Rust permits associating traits with default `const`s, `fn`s, but not `type`s. Allowing `type`s to have defaults helps crate authors provide more ergonomic APIs.
    </Fragment>
    <Fragment slot="example">
        ```rust
    ...
    <Fragment slot="references">
        - [Accepted RFC](https://rust-lang.github.io/rfcs/2532-associated-type-defaults.html)
        - [Tracking issue](https://github.com/rust-lang/rust/issues/29661)
    </Fragment>
</FeatureSummary>

<FeatureSummary title="AsyncDrop">
    <Fragment slot="motivation">
        It is currently impossible to create types (e.g. database connections) that perform async operations on cleanup. We introduce `AsyncDrop` as a async version of `Drop`.
    </Fragment>
    <Fragment slot="example">
        ```rust
        #![feature(async_drop)]
    ...
    <Fragment slot="references">
        - [std::future::AsyncDrop](https://doc.rust-lang.org/stable/std/future/trait.AsyncDrop.html)
        - [Design notes](https://rust-lang.github.io/async-fundamentals-initiative/roadmap/async_drop.html)
        - [Accepted MCP](https://github.com/rust-lang/compiler-team/issues/727)
        - [Tracking issue](https://github.com/rust-lang/rust/issues/126482)
    </Fragment>
</FeatureSummary>

<FeatureSummary title="AsyncIterator">
    <Fragment slot="motivation">
        It often desirable to want an async version of `Iterator`. We introduce `AsyncIterator` as a core async abstraction, using the design from the `Stream` trait defined in the [futures](https://crates.io/crates/futures) crate. Compared to `Iterator`, it allows other tasks to run while it waits rather than blocking between each item yield.
    </Fragment>
    <Fragment slot="example">
        ```rust
    ...
    <Fragment slot="references">
        - [std::async_iter::AsyncIterator](https://doc.rust-lang.org/stable/std/async_iter/trait.AsyncIterator.html)
        - [Accepted RFC](https://github.com/rust-lang/rfcs/pull/2996)
...
<FeatureSummary title="#[cfg(accessible(..) / version(..))]">
    ...
    <Fragment slot="references">
        - [Accepted RFC](https://rust-lang.github.io/rfcs/2523-cfg-path-version.html)
        - [Tracking issue for #[cfg(accessible($path))]](https://github.com/rust-lang/rust/issues/64797)
        - [Tracking issue for #[cfg(version(\<semver>))]](https://github.com/rust-lang/rust/issues/64796)
    </Fragment>
</FeatureSummary>
...
<FeatureSummary title="Exclusive">
    <Fragment slot="motivation">
