...
import secretContext from "../../assets/blog/true-private-state-in-javascript-a-chromium-rabbit-hole/secret-context.png";
...
Such is the case with C++, C#, and so on — there's always a way to alias into private fields in such a way that breaks the requirements for true private state, most namely through foreign function interfaces.
...
Here is an implementation of private state in JavaScript that satisfies all five of my requirements. It utilizes IndexedDB on a Chrome extension background script to store secret variables in an isolated execution context.
...
We have our answer. There's no way for code on the web page to "directly access" IndexedDB. The secret can only be retrieved indirectly, through the `get` method.
...
For no reason at all, following the theme of this blog post, let's make this even more unnecessarily complicated. Utilizing the native application API, it's possible to store the secret outside of Chrome entirely! I've documented my exact findings in this [GitHub repository](https://github.com/ArhanChaudhary/chrome-secret-js). Instead of providing the code's source like before, I had some free time and thought it would be cool to diagram how it worked. Some eye candy:

<ContentImage src={secretDiagram} alt="An image diagramming the native app secret implementation" quality="max" />

# Closing thoughts

It's time to address the elephant in the room. The reason why basically this entire blog post isn't even remotely practical is because of cryptography and networking services on the front-end.

First, cryptography. Take your pick. [WebAuthn large blobs](https://webauthn-large-blob.glitch.me/) and [credential management API](https://developer.mozilla.org/en-US/docs/Web/API/Credential_Management_API) use your operation system keychain to store credentials, with some obscure [limitations](https://issues.chromium.org/issues/40283676#comment5). The [browser.secureStorage](https://github.com/w3c/webextensions/issues/154) API might become a reality soon. SubtleCrypto can be [used with](https://www.w3.org/TR/WebCryptoAPI/#concepts-key-storage) IndexedDB for cryptographically secure key management. Common password managers use this method.
...
So... was this blog basically pointless? Yes. I will openly admit that my requirements for "true" private state seem arbitrary. And I will also accept criticism that my blog deviated from the original intention behind object-oriented programming towards the end. Was this a waste of your time? Maybe. Hopefully you learned something about JavaScript or DevTools debugging or the theory behind object-oriented programming. If you didn't then I don't really care. Until next time!