...
To my credit, this was a surprisingly witty solution. It avoided the use of closures and utilized the `Function` constructor to hide the secret. Calling `valueOf` or `toString` on a function normally reveals it source code, however postfixing two `.bind()`s tricks DevTools into revealing `function () { [native code] }` instead.
