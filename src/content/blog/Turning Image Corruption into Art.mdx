...
It's not quite static noise, and it's not quite order either. It's like you're trying to remember something, but you can only see a blurry mess of pixels. On the technical side, WEBP worked without issue and was well-behaved. I don't have much else to say, so let's finally move on to AVIF.

<ContentVideo
    src={avifs}
/>

This took me many tries to get right. It's my favorite one! There is more detail than meets the eye; you could pause at any timestamp and land on a novel expression of color. It's incredibly mesmerizing,
